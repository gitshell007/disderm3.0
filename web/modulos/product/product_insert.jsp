...
<%@ page import="com.disderm.service.ProductService" %>
...
<%
    ...
    String user_icon = "/backoffice/assets_intranet/dosaolivar/dosaolivar_user.png";
    ...
    if (request.getParameter("isSubmit") != null) {
        ...
        int category_id = Integer.valueOf(request.getParameter("product_category"));
        String description = request.getParameter("description");
        int import_source = Integer.valueOf(request.getParameter("import_source"));
        int cn = Integer.valueOf(request.getParameter("cn"));

        ProductService PS = new ProductService();
        resultService = PS.serviceInsertProduct(category_id, description, import_source, cn);
...
<head>
    ...
    <meta charset="UTF-8"/>
    <link rel="icon" href="<c:url value='/assets/images/dosaolivar/favicon/favicon.ico'/>" type="image/ico"/>
    <title>Sales</title>
    <!--<meta name="viewport"
            ...
            <div class="app-main__inner">
                <div class="header-mobile-wrapper">
                    <div class="app-header__logo">
                        <a href="#" data-toggle="tooltip" data-placement="bottom" title="DOSAOlivar Admin"
                           class="logo-src"></a>
                ...
                <%=MainView.getTopBar("Product insert", "Formulario de introducci&oacute;n para Sales", visible_name, username, user_icon)%>
                <!-- END HEADER TOP BAR -->
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="main-card mb-3 card">
                                                <div class="card-body">
                                                    <!-- CONTENIDO -->
                                                    <form id="productInsertForm" name="productInsertForm">
                                                        ...
                                                        <div class="position-relative row form-group"><label
                                                                for="description"
                                                                class="col-sm-2 col-form-label">CN</label>
                                                            <div class="col-sm-10">
                                                                <input name="cn" id="cn" class="form-control" type="number">
                                                            </div>
                                                        </div>
                                                        <div class="position-relative row">
                                                            <div class="col-sm-12">
                                                                <button class="btn btn-primary form-group"
                                                                        onclick="productInsertAction()"
                                                                        style="width: inherit;">
                                                                    Insertar
                                                                </button>
...
<script>
    ...
    const form = $("#productInsertForm");
    const lockModal = $("#lock-modal");
    const loadingCircle = $("#loading-circle");
    var productInsertAction = function () {
...
/*
    let dropdownCn = $('#cn');
    const url_cn = 'http://localhost:8080/feelfarma/services/cn/getSelectBootstrap/';
    function loadSelectCn(){
        dropdownCn.append($('<option selected disabled hidden></option>').attr('value', '0').text('Seleccione Valor'));
        const mock = [{"dropdown_id":1, "code": 0, "des":"ERROR"}];
        genericFetch(url_cn, dropdownCn, mock);
    }
    $.getJSON(url_cn, function (data) {
        $.each(data, function (key, entry) {
            dropdownCn.append($('<option></option>').attr('value', entry.code).text(entry.des));
        })
    })*/
    ...
    let dropdownCategory = $('#product_category');
    const url_category = 'http://localhost:8080/feelfarma/services/product_category/getSelectBootstrap/';
    ...
    $(document).ready(() => {
        ...
        $('#avisoBorrarModal').on('hidden.bs.modal', function () {
            console.log('x');
            window.location.replace("/backoffice/modulos/product/tbl_custom_search_product.jsp");
        });

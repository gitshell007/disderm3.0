                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="main-card mb-3 card">
                                                <div class="card-body">
                                                    <h3 class="title">Diagnósticos pendientes</h3>
                                                    ...
                                                    <h3 class="title">Diagnósticos con incidencia</h3>
                                                    <table style="width: 100%;" id="tblAgenda2"
                                                           class="table table-hover table-striped table-bordered">
                                                        <thead>
                                                        <tr>
                                                            <th>ID</th>
...
<script>
    ...
    $(document).ready(function () {
            ...
            ajax: "/backoffice/services/diagnostico/getCustomDataTable_diagnosticos_pendientes",
            aoColumns: [
        ...
        var CrearConcepto = function () {
            ...
            var input2 = $("#pacienteID")
            input.val(selData[0][0]);
            input2.val(selData[0][1])
        ...
        table_Agenda.on('dblclick', 'tr', function () {
            var selData = table_Agenda.rows(this).data();
            var form_main = $("#diagnostico_id")
            var input = $("#diagnosticoID")
            var input2 = $("#pacienteID")
            input.val(selData[0][0]);
    ...
    var table_Agenda2;
    $(document).ready(function () {
        table_Agenda2 = $('#tblAgenda2').DataTable({
            dom: 'Bfrtip',
            processing: true,
            searching: true,
            serverSide: true,
            ...
            ajax: "/backoffice/services/diagnostico/getCustomDataTable_diagnosticos_incidencias",
            aoColumns: [
                {"bSortable": true, "bSearchable": false, "bVisible": false},
                {"bSortable": true, "bSearchable": false},
                {"bSortable": false, "bSearchable": false},
        ...
        table_Agenda2.on('select deselect', function () {
            var selectedRows = table_Agenda.rows({selected: true}).count();
            //console.log('x' + selectedRows);
            table_Agenda.button(1).enable(selectedRows === 1);
        });

        table_Agenda2.on('dblclick', 'tr', function () {
            var selData = table_Agenda.rows(".selected").data();
            var form_main = $("#diagnostico_id")
            var input = $("#diagnosticoID")
            var input2 = $("#pacienteID")
        ...
        var CrearConcepto2 = function () {
            var selData = table_Agenda2.rows(".selected").data();
            var form_main = $("#diagnostico_id")
            var input = $("#diagnosticoID")
            var input2 = $("#pacienteID")
            input.val(selData[0][0]);
        ...
        var eliminarConcepto2 = function () {
            var selData = table_Agenda2.rows(".selected").data();
            var form_main = $("#eliminar_diagnostico")
            var input = $("#eliminar_diagnostico_id")
            input.val(selData[0][0]);
        ...
        table_Agenda2.button(0).nodes().css('background', '#b42d8f');
        table_Agenda2.button(0).nodes().css('color', '#ffffff');
        table_Agenda2.button(1).nodes().css('background', '#b42d8f');
        table_Agenda2.button(1).nodes().css('color', '#ffffff');

        $('#tblAgenda_next').removeAttr('disabled');

        TBL_AGENDA_TBL = $('#tblAgenda2').DataTable();
    })
